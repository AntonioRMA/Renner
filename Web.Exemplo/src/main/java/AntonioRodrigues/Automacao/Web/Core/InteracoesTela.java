...
//            WebDriver.quit();
    ...
    protected boolean ClicarBotao(LocalizaPor tipoLocalizador, String localizador) {
        ...
        while (!retorno.Existe && limitador > 0) {
        	limitador--;
        	try {
				wait(2000);
			} catch (Exception e) {
        	...
        	retorno = this.ObterElemento(tipoLocalizador, localizador);
        }
    ...
    protected boolean SelecionarComboPorValor(LocalizaPor tipoLocalizador, String localizador, String texto) {
        Log.LogarInfo("Inserindo texto no campo: ".concat(localizador));
        Elemento retorno = this.ObterElemento(tipoLocalizador, localizador);

        if (retorno.Existe) {
        	retorno.Instancia.click();
        }
    ...
    protected String ObterTexto(LocalizaPor tipoLocalizador, String localizador) {
        ...
        while (!elemento.Existe && limitador > 0) {
        	limitador--;
        	try {
				wait(2000);
        	...
        	elemento = this.ObterElemento(tipoLocalizador, localizador);
        }
    ...
    protected List<List<String>> ObterLinhasTabela(LocalizaPor tipoLocalizador, String localizador){
        Log.LogarInfo("Linhas da tabela: ".concat(localizador));
        ...
        for (WebElement row : elementos.Instancias) {
        	List<WebElement> cells = row.findElements(By.tagName("td"));
        	
            for (WebElement cell : cells) 
            	if (null != cell && cell.getText().length() > 0)
            		linhas.add(cell.getText());
